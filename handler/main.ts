...
enum MysqlErrorCodes {
    // TODO
    DUPLICATE_DATABASE = 'ER_DB_CREATE_EXISTS',
    DUPLICATE_OBJECT = '2',
...
// Class that lazily creates a Mysql client from a secret ARN,
// dbClusterHostname, dbClusterPort and databaseName, and caches the client
interface LazyMysqlClientFromSecretsManagerProps {
    dbSecretArn: string;
...
class LazyMysqlClientFromSecretsManager {
    private props: LazyMysqlClientFromSecretsManagerProps;
    private connection?: mysql.Connection;
    private credentials?: DbCredentials;

    constructor(props: LazyMysqlClientFromSecretsManagerProps) {
        this.props = props;
    }
    ...
    async getClient(): Promise<mysql.Connection> {
        if (this.connection) {
...
const handleCreate = async (event: CreateEvent): Promise<Response> => {
    log('Handling create');
    const adminClientManager = new LazyMysqlClientFromSecretsManager({
    ...
    const userClientManager = new LazyMysqlClientFromSecretsManager({
...
const handleUpdate = async (event: UpdateEvent): Promise<Response> => {
    log('Handling update');
    const userClient = new LazyMysqlClientFromSecretsManager({
        dbSecretArn: event.ResourceProperties.userSecretArn,
        dbClusterHostname: event.ResourceProperties.dbClusterHostname,
        dbClusterPort: event.ResourceProperties.dbClusterPort,
    });
    ...
    const adminClient = new LazyMysqlClientFromSecretsManager({
        dbSecretArn: event.ResourceProperties.dbSecretArn,
        dbClusterHostname: event.ResourceProperties.dbClusterHostname,
        dbClusterPort: event.ResourceProperties.dbClusterPort,
    });
    ...
    if (event.ResourceProperties.onUpdateIfUserDoesNotExist === 'Create') {
        ...
        try {
            ...
            if (e.code !== MysqlErrorCodes.DUPLICATE_OBJECT) {
                throw e;
            } else {
